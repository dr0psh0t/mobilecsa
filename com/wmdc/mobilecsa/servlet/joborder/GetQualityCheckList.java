...
public class GetQualityCheckList extends HttpServlet {
    private String getQcList(ServletContext context) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!checkParameters(serverUrl, akey, cid, source, page, resJson, out, ctx)) {
            return;
        }
        ...
        try {
            ...
            Utils.printJsonException(new JSONObject(), "Cannot get quality check list at this time.", out);
            Utils.displayStackTraceArray(sqe.getStackTrace(), Utils.JOBORDER_PACKAGE, "NetworkException",
                    sqe.toString(), ctx, null);

        } catch (Exception e) {
            Utils.printJsonException(new JSONObject(), "Cannot get quality check list at the moment.", out);
    ...
    public boolean checkParameters(String serverUrl, String akey, String cid, String source, String page,
        ...
        if (page == null) {
            Utils.logError("\"page\" parameter is empty", ctx);
            Utils.printJsonException(resJson, "Missing data required. Try again or see logs.", out);
            return false;
